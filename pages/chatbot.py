...
# -------------------------------
# Load Data from finance.db (WITH TIMEZONE FIX)
# -------------------------------
...
def get_recurring_audit(df, exchange_rate, currency_symbol):
    ...
    for cat, row in monthly_avg.iterrows():
        ...
        latest_date = latest_tx[latest_tx['Category'] == cat]['Date'].iloc[0].date()
        days_since = (today - latest_date).days
        if days_since > 60:
            lines.append(f"⚠️ <strong>{cat}</strong>: {currency_symbol}{avg_amt:,.2f}/month <em>(last used {days_since} days ago)</em>")
        else:
