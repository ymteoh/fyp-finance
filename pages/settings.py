...
# -------------------------------
    ...
    # FIRST TIME: Auto-download from Google Drive (WORKS 100%)
    if not os.path.exists(forecast_dir):
        st.info("First time setup: Downloading AI forecast models from Google Drive... (30-60s only once)")

        # CORRECT FILE ID (NO ?usp=sharing!)
        GOOGLE_DRIVE_FILE_ID = "19sjbcmLfi_Xw_DLYPQVnf2dfyo9E_UEH"   # ← YOUR REAL ID
        ...
        try:
            ...
            st.success("AI Forecast Models Downloaded Successfully!")
            st.balloons()
            st.rerun()
        except Exception as e:
            st.error(f"Download failed: {e}")
            st.info("Check that your file is shared as 'Anyone with the link'")
            st.stop()
